ZIP_NAME ?= "customDataTypeTNADiscovery.zip"
PLUGIN_NAME = "custom-data-type-tnadiscovery"

# coffescript-files to compile
COFFEE_FILES = CustomDataTypeTNADiscovery.coffee \
	CustomDataTypeTNADiscoveryFacet.coffee \
	TNADiscoveryUtil.coffee
...
build: clean buildinfojson ## clean, compile, copy files to build folder
			...
			cat src/tmp/CustomDataTypeTNADiscovery.js >> build/$(PLUGIN_NAME)/webfrontend/customDataTypeTNADiscovery.js
			cat src/tmp/CustomDataTypeTNADiscoveryFacet.js >> build/$(PLUGIN_NAME)/webfrontend/customDataTypeTNADiscovery.js
			cat src/tmp/TNADiscoveryUtil.js >> build/$(PLUGIN_NAME)/webfrontend/customDataTypeTNADiscovery.js

			cp src/updater/TNADiscoveryUpdater.js build/$(PLUGIN_NAME)/updater/TNADiscoveryUpdater.js # build updater
			cat src/tmp/TNADiscoveryUtil.js >> build/$(PLUGIN_NAME)/updater/TNADiscoveryUpdater.js
			cp package.json build/$(PLUGIN_NAME)/package.json
			cp -r node_modules build/$(PLUGIN_NAME)/
			rm -rf src/tmp # clean tmp

			cp l10n/customDataTypeTNADiscovery.csv build/$(PLUGIN_NAME)/l10n/customDataTypeTNADiscovery.csv # copy l10n			
			
			cp src/webfrontend/css/main.css build/$(PLUGIN_NAME)/webfrontend/customDataTypeTNADiscovery.css # copy css
